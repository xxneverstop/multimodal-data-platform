<template>
  <div class="space-y-4">
    <PageHeader eyebrow="功能 / 质检" title="质检" description="查看质检规则和质检日志。" />
        ...
        <table class="min-w-full text-left text-sm">
          <thead class="bg-slate-50 text-xs text-slate-500">
            <tr>
              ...
              <th class="px-3 py-2.5 font-medium">模板</th>
              <th class="px-3 py-2.5 font-medium">作用域</th>
          ...
          <tbody class="divide-y divide-slate-200 bg-white">
            <tr v-for="item in filteredRules" :key="item.id" class="hover:bg-slate-50">
              ...
              <td class="px-3 py-2.5 text-slate-600">{{ item.templateName }}</td>
              <td class="px-3 py-2.5 text-slate-600">{{ item.scope }}</td>
        ...
        <table class="min-w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-slate-200 bg-white">
            <tr v-for="item in filteredLogs" :key="item.id" class="hover:bg-slate-50">
              ...
              <td class="px-3 py-2.5 text-right"><BaseButton size="sm" variant="ghost" @click="selectedLog = item">详情</BaseButton></td>
            </tr>
          </tbody>
        </table>
      </DataTableShell>
    ...
    <AppDialog :open="dialogOpen" title="新建质检规则" description="先保留前端占位字段。" @close="dialogOpen = false" @confirm="submitMock('质检规则已创建')">
      <div class="grid gap-4 md:grid-cols-2">
        ...
        <label class="block">
          <span class="mb-1 block text-sm text-slate-600">规则模板</span>
          <input v-model="dialogForm.template" class="app-input app-input-compact" />
        </label>
      </div>
...
const dialogForm = reactive({ name: "", template: "" });
const filterFields: FilterField[] = [
  { key: "taskId", label: "taskId", placeholder: "请输入任务编号" },
  { key: "sessionId", label: "sessionId", placeholder: "请输入会话编号" }
];
const filteredRules = computed(() => rules.value.filter((item) => (!filters.taskId || String(item.taskId ?? "").includes(filters.taskId.trim())) && (!filters.sessionId || String(item.sessionId ?? "").includes(filters.sessionId.trim()))));
const filteredLogs = computed(() => logs.value.filter((item) => (!filters.taskId || String(item.taskId).includes(filters.taskId.trim())) && (!filters.sessionId || item.sessionId.includes(filters.sessionId.trim()))));
