...
public interface CollectionSessionMapper extends BaseMapper<CollectionSession> {
    ...
    @Select({
            ...
            " COALESCE(NULLIF(c.client_name, ''), NULLIF(c.client_code, ''), NULLIF(t.operator_name, ''), '手动上传') AS collectorName,",
            " COALESCE(fileAgg.uploaded_at, s.updated_at, s.created_at) AS uploadedAt,",
            ...
            "LEFT JOIN collector_client c ON c.id = s.collector_client_id",
            "LEFT JOIN (",
            "  SELECT f.session_id AS session_id, COUNT(*) AS file_count, COALESCE(SUM(f.file_size), 0) AS total_size, MAX(f.created_at) AS uploaded_at",
            "  FROM data_file f",
