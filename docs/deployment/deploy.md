...
| Worker 依赖 | `depends_on: mmdp-backend` | **必须先启动 Backend 再启动 Worker**。Worker 启动时向 Backend 注册 Pipeline 清单，若 Backend 未就绪则注册失败需等下次心跳重试（60s），期间提交处理任务会被拒绝 |
...
6. Backend 启动后 → Worker 启动 → 向 Backend 注册 Pipeline 清单 → 开始轮询领取任务
7. Nginx 启动，绑定 80/443 端口

> **启动顺序说明**：Backend 必须先于 Worker 启动。Worker 启动时会调用 `POST /api/worker/pipelines/register` 将自身 Pipeline 清单注册到 Backend 内存注册表。若 Backend 未就绪导致注册失败，Worker 会在下次心跳（60s 后）重试，但此期间前端提交处理任务会被拒绝并提示 "Pipeline 未在 Worker 端注册"。
...
| **Worker 报 `Unknown pipeline`** | Pipeline 未在 Worker 端注册 | `docker exec mmdp-worker python main.py --list-pipelines` 确认已注册列表；检查 Worker 是否在 Backend **之后**启动；确认 `pipeline_definition` 表中 pipelineId 与 Python 代码中的 `pipeline_id` 完全一致（含大小写） |
| **前端提交任务报 "Pipeline 未在 Worker 端注册"** | Worker 未启动或注册失败 | ① 确认 Worker 容器运行中；② 查看 Worker 日志是否有 `[register] 已向 Backend 注册`；③ 若 Worker 在 Backend 之前启动，等待 60s 心跳重试或重启 Worker |
