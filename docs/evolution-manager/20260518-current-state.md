...
1. 原有主链路  
   `任务创建 -> 文件上传 -> MinIO 存储 -> data_file 入库 -> 基础 QC -> qc_report 入库 -> QC 报告查询`
...
这条链路只登记元数据，不上传文件，不走 MinIO，不写 `data_file`。
...
后端当前已经完成“任务 + 上传文件 + MinIO + 基础 QC + 数据资产 + pipeline readiness + processing job”的真实业务骨架，但其中 QC 仍主要是最初的基础检查逻辑，processing job 的算法执行仍是占位实现。
